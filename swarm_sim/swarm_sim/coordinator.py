...
YAW_TOL = 0.08                # ~4.5 deg: heading band a parked follower keeps
...
def angle_diff(a, b):
    """Shortest signed rotation from b to a, in [-pi, pi]."""
...
class Robot:
    ...
    def drive_to(self, target, tol, max_lin, final_yaw=None):
        """Proportional go-to-point. Once inside tol, rotate in place toward
        final_yaw (if given) instead of simply stopping, so a parked follower
        stays aligned with the formation and does not have to turn from
        scratch when the leader moves off. Returns True when in position.
        """
        ...
        if dist < tol:
            if final_yaw is not None:
                yaw_err = angle_diff(final_yaw, yaw)
                if abs(yaw_err) > YAW_TOL:
                    cmd.angular.z = clamp(K_ANG * yaw_err, -MAX_ANG, MAX_ANG)
            self.cmd_pub.publish(cmd)
            return True

        err = angle_diff(math.atan2(dy, dx), yaw)
...
class SwarmCoordinator(Node):
    ...
    frame and hold the leader's heading once in position. In M3 the LLM
    supervisor replaces LEADER_WAYPOINTS and FORMATION with values derived
